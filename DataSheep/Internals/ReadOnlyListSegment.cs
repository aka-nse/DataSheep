...
internal class ReadOnlyListSegment<T>(IReadOnlyList<T> source, int start, int count) : IReadOnlyList<T>
{
...
internal static class ReadOnlyListSegment
{
    ...
    public static IReadOnlyList<T> Slice<T>(this IReadOnlyList<T> list, int start, int count)
        => new ReadOnlyListSegment<T>(list, start, count);
}