    ...
    : IMutableSeries, ISeries<T>
{
    ...
    internal ArraySeries(string columnName, int initialMinimumCapacity)
    {
        ColumnName = columnName;
        _array = new T[Math.Max(256, BitOperations.RoundUpToPowerOf2((uint)initialMinimumCapacity))];
    }

    internal ArraySeries(string columnName, ReadOnlySpan<T> initialValues)
    {
        ColumnName = columnName;
